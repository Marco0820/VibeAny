...
<img src="./assets/VibeAny.png" alt="VibeAny" style="width: 100%;" />
<div align="center">
...
VibeAny is a powerful Next.js-based web app builder that combines **C**laude Code's (Cursor CLI also supported!) advanced AI agent capabilities with VibeAny's own simple and intuitive app building experience. Just describe your app idea - "I want a task management app with dark mode" - and watch as VibeAny instantly generates the code and shows you a live preview of your working app. You can deploy your app to Vercel and integrate database with Supabase for free.

This open-source project empowers you to build and deploy professional web applications easily for **free**.

How to start? Simply login to Claude Code (or Cursor CLI), start VibeAny, and describe what you want to build. That's it. There is no additional subscription cost for app builder. 

## Features
<img src="./assets/gif/VibeAny_v2_cc_4_1080p.gif" alt="VibeAny Demo" style="width: 100%; max-width: 800px;">
...
- **Natural Language to Code**: Simply describe what you want to build, and VibeAny generates production-ready Next.js code
...
### Codex CLI Example
<img src="./assets/gif/VibeAny_v2_codex_1_1080p.gif" alt="Codex CLI Demo" style="width: 100%; max-width: 800px;">

### Qwen Code Example
<img src="./assets/gif/VibeAny_v2_qwen_1_1080p.gif" alt="Qwen Code Demo" style="width: 100%; max-width: 800px;">

## Supported AI Coding Agents

VibeAny supports multiple AI coding agents, giving you the flexibility to choose the best tool for your needs:
...
Get VibeAny running on your local machine in minutes:

```bash
# Clone the repository
git clone https://github.com/opactorai/VibeAny.git
cd VibeAny
...
- 支付渠道配置说明请参考 `docs/payments-setup.md`，内含 Stripe / Creem / PayPal 的密钥获取及环境变量示例。
- 计费维护脚本位于 `apps/api/app/scripts/billing_tasks.py`，可执行：

  ```bash
  cd apps/api
  # 重置 Free 用户 Auto-fix 次数 & 更新 BudgetGuard
  python -m app.scripts.billing_tasks --task daily
  ```

  建议将 `--task daily` 加入定时任务（例如 cron）在每天 00:00 UTC 后运行一次。
...
Set up Google and GitHub OAuth applications before enabling social login in VibeAny.

1. **Create OAuth apps**
   - Google: Follow the [Google Identity Services quickstart](https://developers.google.com/identity/gsi/web/guides/display-google-one-tap) to create OAuth credentials and authorize your callback URL.
   - GitHub: Follow GitHub's [Authorizing OAuth Apps guide](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/authorizing-oauth-apps) to register the application.
2. **Configure callback URLs**
   - Set authorized redirect URIs to `http://localhost:8080/api/auth/callback/google` and `http://localhost:8080/api/auth/callback/github` for local development. Use your deployed API domain in production.
3. **Populate environment variables**
   ...
   ```
4. **Restart the servers** so the new credentials are loaded.
...
VibeAny uses SQLite for local development and can be configured for PostgreSQL in production. The database automatically initializes on first run.
...
2. Ensure proper file ownership in WSL:
   ...
   sudo chown -R $(whoami):$(whoami) ~/VibeAny
   ```
...
[![Star History Chart](https://api.star-history.com/svg?repos=opactorai/VibeAny&type=Date)](https://www.star-history.com/#opactorai/VibeAny&Date)
