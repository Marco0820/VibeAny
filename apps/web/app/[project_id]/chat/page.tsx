...
const hexToFilter = (hex: string): string => {
  ...
  const filters: { [key: string]: string } = {
    '#0F172A': 'brightness(0) saturate(100%) invert(8%) sepia(10%) saturate(1878%) hue-rotate(182deg) brightness(95%) contrast(92%)',
...
export default function ChatPage({ params }: Params) {
  ...
  const [customPreviewInput, setCustomPreviewInput] = useState('');
  ...
  useEffect(() => {
    ...
    const storedUrl = localStorage.getItem(`project_${projectId}_customPreviewUrl`);
    if (storedUrl) {
      setCustomPreviewInput(storedUrl);
      setCustomPreviewUrl(storedUrl);
      setPreviewUrl(storedUrl);
    } else {
      setCustomPreviewInput('');
  ...
  const [preferredCli, setPreferredCli] = useState<string>('remote');
  ...
  const applyCustomPreview = useCallback(() => {
    if (typeof window === 'undefined') return;
    const trimmed = customPreviewInput.trim();

    if (!trimmed) {
      localStorage.removeItem(`project_${projectId}_customPreviewUrl`);
      setCustomPreviewInput('');
      setCustomPreviewUrl(null);
      setPreviewUrl(null);
      setPreviewInitializationMessage('Custom preview cleared.');
    ...
    try {
      ...
      setPreviewError({
        message: 'Please enter a valid preview URL (for example http://127.0.0.1:3300).',
        details: trimmed
      });
      return;
    }

    localStorage.setItem(`project_${projectId}_customPreviewUrl`, url);
    setCustomPreviewUrl(url);
    setCustomPreviewInput(url);
    setPreviewUrl(url);
    setPreviewInitializationMessage('Using custom preview URL.');
    setPreviewError(null);
  ...
  const handlePreviewEvent = useCallback((event: any) => {
    if (!event) return;
    if (event.type === 'preview_error') {
      const error = event.error || {};
      if (error.kind === 'permission') {
        setPreviewError({
          message: 'The current environment blocks local ports so the preview cannot start automatically. Run `npm run dev -- --port 3300 --hostname 127.0.0.1` in your terminal and enter the custom preview URL below.',
          details: error.context
        });
      } else {
        setPreviewError({
          message: error.message || 'Failed to launch the preview service. Please check the logs.',
          details: error.context
        });
      }
      setIsStartingPreview(false);
    } else if (event.type === 'preview_success') {
      setPreviewError(null);
    }
  }, []);
  ...
  const handleExportProject = useCallback(async () => {
    try {
      ...
      link.href = url;
      link.download = `${(projectName || projectId).replace(/\s+/g, '-')}.zip`;
  ...
  async function start() {
    try {
      ...
      if (!r.ok) {
        ...
        setPreviewError({
          message: 'Unable to start the preview automatically. See the tips below or use a custom preview URL.',
          details: detail
        });
      ...
      setPreviewError({
        message: 'Unable to start the preview service. Check the logs or provide a custom preview URL.',
        details: String(error)
      });
      setTimeout(() => setIsStartingPreview(false), 2000);
    }
    ...
    try {
      ...
      if (!hasCliSet || !hasModelSet) {
        ...
        if (globalResponse.ok) {
          ...
          const defaultCli = globalSettings.default_cli || 'remote';
          ...
          if (!hasModelSet) {
            ...
            if (cliSettings?.model) {
              ...
              setSelectedModel(defaultModelForCli(currentCli));
            }
          }
        } else {
          ...
          if (response.ok) {
            ...
            if (!hasCliSet) setPreferredCli(settings.preferred_cli || 'remote');
            if (!hasModelSet) setSelectedModel(defaultModelForCli(settings.preferred_cli));
          }
        }
      }
    } catch (error) {
      ...
      const hasCliSet = projectSettings?.cli || preferredCli;
      const hasModelSet = projectSettings?.model || selectedModel;
      if (!hasCliSet) setPreferredCli('remote');
      if (!hasModelSet) setSelectedModel('gpt-5');
    }
  }
  ...
  useEffect(() => {
    ...
    const cli = globalSettings.default_cli || 'remote';
    setPreferredCli(cli);
        ...
        <div className="h-full w-full flex">
          ...
          >
            ...
            <div className="p-4 rounded-bl-2xl">
              <ChatInput 
                ...
                disabled={isRunning}
                placeholder={mode === 'act' ? "Ask VibeAny..." : "Chat with VibeAny..."}
                mode={mode}
          ...
          <div className="h-full flex flex-col bg-black" style={{ width: '70%' }}>
            ...
            <div className="flex-1 min-h-0 flex flex-col">
              ...
              {showPreview && (
                <div className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-4 py-3 space-y-2">
                  {previewError && (
                    <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/30 px-3 py-2 text-sm text-amber-800 dark:text-amber-200">
                      <p>{previewError.message}</p>
                      {previewError.details && (
                        <details className="mt-2 text-xs text-amber-700 dark:text-amber-300">
                          <summary className="cursor-pointer">View details</summary>
                          <pre className="mt-1 whitespace-pre-wrap break-all">{previewError.details}</pre>
                        </details>
                      )}
                    </div>
                  )}
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm text-gray-600 dark:text-gray-300">Custom preview URL</span>
                    <input
                      type="text"
                      value={customPreviewInput}
                      onChange={(e) => setCustomPreviewInput(e.target.value)}
                      placeholder="http://127.0.0.1:3300"
                      className="h-9 w-72 max-w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 text-sm text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/70"
                    />
                    <button
                      className="h-9 px-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors"
                    ...
                    {customPreviewUrl && (
                      <button
                        className="h-9 px-3 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-lg text-sm font-medium transition-colors hover:bg-gray-300 dark:hover:bg-gray-600"
                        onClick={clearCustomPreview}
                      >
                        Clear
                      </button>
                    )}
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Run <code className="font-mono">npm run dev -- --port 3300 --hostname 127.0.0.1</code> locally, then paste the preview URL here to open the generated app in your browser.
                  </p>
                </div>
              )}
                ...
                {previewUrl ? (
                  ...
                  <div className="h-full w-full flex items-center justify-center bg-gray-50 dark:bg-black relative">
                    ...
                    {isStartingPreview ? (
                      ...
                      >
                        {/* VibeAny Symbol with loading spinner */}
