...
export default function GitHubRepoModal({ 
  ...
  const baseRepoName = sanitizeRepoName(projectName || projectId || 'project') || 'project';
  const [suggestedRepoName, setSuggestedRepoName] = useState(() => sanitizeRepoName(`${baseRepoName}-starter`));
  ...
  useEffect(() => {
    ...
    setSuggestedRepoName(sanitizeRepoName(`${baseRepoName}-${randomSuffix}`));
  }, [isOpen, baseRepoName]);
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      setIsPrivate(false);
      setIsCheckingAvailability(false);
      setSuggestedRepoName(sanitizeRepoName(`${baseRepoName}-starter`));
    }
  }, [isOpen, baseRepoName]);
                ...
                {!nameError && !isCheckingAvailability && (
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    Great repository names are short and memorable. Need inspiration? How about <button type="button" className="text-gray-900 dark:text-white hover:underline" onClick={() => {
                      const randomSuffix = Math.random().toString(36).substring(2, 9);
                      const suggestion = sanitizeRepoName(`${baseRepoName}-${randomSuffix}`);
                      setSuggestedRepoName(suggestion);
