...
export default function Header() {
  ...
  const isLandingHeaderRoute = pathname === '/vibeany' || pathname === '/pricing';
  ...
  return (
    <>
      {isLandingHeaderRoute && <LandingHeader />}
      {!isMarketingPage && (
        <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-30 transition-colors duration-200">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                {!loading && user && (
                  <div className="flex items-center gap-3">
                    ...
                    <div className="hidden sm:flex flex-col">
                      <span className="text-sm font-medium text-gray-800 dark:text-gray-200">{user.name || user.email || 'VibeAny User'}</span>
                      <span className="text-xs text-gray-500 dark:text-gray-400">Lv.{user.level} · {user.points} pts</span>
                    </div>
                    <a
                      href="/account"
                      className="hidden sm:inline-flex items-center px-3 py-1.5 text-sm font-medium text-white bg-gray-900 dark:bg-white dark:text-gray-900 rounded-lg hover:bg-gray-700 dark:hover:bg-gray-200 transition-colors"
                    >
                      Account
                    </a>
                    <button
                      onClick={() => logout('/')}
                      className="inline-flex items-center px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 border border-gray-200 dark:border-gray-700 rounded-lg transition-colors"
                    >
                      Sign out
                    </button>
                  </div>
                ...
                {!loading && !user && (
                  <button
                    onClick={() => openLogin()}
                    className="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-lg transition-colors bg-gray-900 hover:bg-gray-800 text-white dark:bg-white dark:text-gray-900 dark:hover:bg-gray-200"
                    type="button"
                  >
                    Sign in
                  </button>
