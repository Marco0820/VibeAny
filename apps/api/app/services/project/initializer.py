...
def _configure_next_config_for_preview(project_path: str) -> None:
    """Update next.config.ts to enable iframe embedding only when explicitly allowed."""
    config_path = Path(project_path) / "next.config.ts"
    ...
    if original_content.strip() != default_stub.strip():
        # Respect user-modified configs; assume they will manage headers themselves.
        return

    config_path.write_text(
        'import type { NextConfig } from "next";\n\n'
        'const allowIframePreview = process.env.NODE_ENV !== "production";\n\n'
        'const nextConfig: NextConfig = {\n'
        '  async headers() {\n'
        '    if (!allowIframePreview) {\n'
        '      return [];\n'
        '    }\n\n'
        "    return [\n"
        "      {\n"
...
async def initialize_project(project_id: str, name: str) -> str:
    ...
    try:
        ...
        # Adjust Next.js config to allow iframe embedding only in dev preview mode
        _configure_next_config_for_preview(project_path)
