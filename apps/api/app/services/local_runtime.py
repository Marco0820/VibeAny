...
def _broadcast_permission_error(project_id: str, message: str, port: Optional[int] = None) -> None:
    """Notify connected clients that preview cannot start due to permission restrictions."""
    # Record the message so it shows up in aggregated logs
    if project_id not in _process_logs:
        _process_logs[project_id] = []
    if message.strip():
        _process_logs[project_id].append(message.strip())
    ...
    try:
        ...
        payload = {
            ...
            "error": {
                ...
                "kind": "permission",
                "message": "Preview server cannot bind to a local port in this environment.",
                "context": message.strip(),
                "port": port,
                "timestamp": int(time.time() * 1000),
                "suggestion": "Run the project locally (npm run dev -- --port 3300 --hostname 127.0.0.1) and set the Custom Preview URL in the UI."
            }
        }
        ...
        try:
            ...
            loop.run_until_complete(manager.send_message(project_id, payload))
        finally:
            try:
                loop.close()
            except Exception:
        ...
        print(f"[PreviewError] Failed to broadcast permission error for {project_id}: {send_error}")

def _monitor_preview_errors(project_id: str, process: subprocess.Popen):
    """Preview server error monitoring loop"""
    from app.core.websocket.manager import manager
    import asyncio
    ...
    recent_errors = {}     # Track last send time per error id
    error_contexts = {}
    current_error = None
    error_lines = []
    
    def generate_error_id(error_line):
        """Create a stable identifier for an error line"""
        import hashlib
        # Remove volatile parts (timestamps, locations)
    ...
    def collect_error_context(line_text):
        ...
        # Persist logs per project for retrieval
        if project_id not in _process_logs:
            _process_logs[project_id] = []
        ...
        # Keep up to 1000 lines of history
        if len(_process_logs[project_id]) > 1000:
            _process_logs[project_id] = _process_logs[project_id][-1000:]
        ...
        for pattern in success_patterns:
            if pattern in line_text:
                ...
                try:
                    ...
                    print("[PreviewSuccess] WebSocket notification sent")
                except Exception as e:
                    print(f"[PreviewSuccess] WebSocket notification failed: {e}")

                current_error = None
                error_lines = []
                return

        # Detect a new error block
    ...
    def send_error_with_context(error_id, lines):
        """Send error information with context lines"""
        ...
        print(f"[PreviewError] Sending error (ID: {error_id}): {main_message[:100]}")
        ...
        try:
            ...
            print(f"[PreviewError] WebSocket notification sent (ID: {error_id})")
        except Exception as e:
            print(f"[PreviewError] WebSocket notification failed: {e}")
    ...
    # Process terminated – flush remaining error
    if current_error and error_lines:
        send_error_with_context(current_error, error_lines)

    print(f"[PreviewError] Monitoring finished for {project_id}")
...
def start_preview_process(project_id: str, repo_path: str, port: Optional[int] = None) -> tuple[str, int]:
    ...
    try:
        ...
        process = subprocess.Popen(
            ["npm", "run", "dev", "--", "--port", str(port), "--hostname", "0.0.0.0"],
            cwd=repo_path,
            env=env,
        ...
        error_thread.start()
        print(f"[PreviewError] Monitoring started for {project_id}")
        ...
        _broadcast_permission_error(project_id, str(e), port)
        raise RuntimeError("Permission denied while starting preview process") from e
    except Exception as e:
        error_message = str(e)
        if "operation not permitted" in error_message.lower():
            _broadcast_permission_error(project_id, error_message, port)
        raise RuntimeError(f"Failed to start preview process: {error_message}") from e
...
def get_all_preview_logs(project_id: str) -> str:
    ...
    # Remove repeated error blocks
    logs = _process_logs[project_id]
    if not logs:
        return "No logs available for this project"
    
    # Remove large duplicate segments
    unique_logs = []
    seen_blocks = set()
    current_block = []
    ...
    for line in logs:
        ...
        # Detect the end of a block (new request or explicit boundary)
