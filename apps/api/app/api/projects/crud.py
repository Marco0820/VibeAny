...
async def export_project_archive(project_id: str, db: Session = Depends(get_db)):
    """Export the project repository as a downloadable zip archive."""

    project = db.query(ProjectModel).filter(ProjectModel.id == project_id).first()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")

    repo_path = project.repo_path or os.path.join(settings.projects_root, project_id, "repo")
    ...
    buffer.seek(0)
    safe_name = (project.name or project_id).replace(" ", "-")
...
async def create_project(
    ...
    project_cost = points_service.get_usage_cost("project_creation")
    if user:
        if project_cost > 0 and not points_service.ensure_balance(user, project_cost):
            raise HTTPException(status_code=402, detail="积分不足，无法创建新项目")
    elif not settings.allow_anonymous_generation:
        # Should not be reached because dependency would raise, but guard for clarity
        raise HTTPException(status_code=401, detail="Not authenticated")

    # Create database record with initializing status
    preferred_cli = body.preferred_cli or "remote"
    # Set default model based on CLI
    selected_model = body.selected_model
    if not selected_model:
        if preferred_cli == "remote":
            selected_model = settings.gpt5_model or "gpt-5"
    ...
    if user and project_cost > 0:
        try:
            ...
            db.delete(project)
            db.commit()
            raise HTTPException(status_code=402, detail="积分不足，项目创建已回滚")
